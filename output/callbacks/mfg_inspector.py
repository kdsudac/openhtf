...
from google3.googlex.warhol.proto import mfg_event_pb2
...
UOM_CODE_MAP = {
    ...
    for num, u in
    test_runs_pb2.Units.UnitCode.DESCRIPTOR.values_by_number.iteritems()
}
...
def _add_log_lines(record, testrun):
  ...
  for log in record.log_records:
    ...
    if log.level <= logging.DEBUG:
      testrun_log.level = mfg_event_pb2.TestRunLogMessage.DEBUG
    elif log.level <= logging.INFO:
      testrun_log.level = mfg_event_pb2.TestRunLogMessage.INFO
    elif log.level <= logging.WARNING:
      testrun_log.level = mfg_event_pb2.TestRunLogMessage.WARNING
    elif log.level <= logging.ERROR:
      testrun_log.level = mfg_event_pb2.TestRunLogMessage.ERROR
    elif log.level <= logging.CRITICAL:
      testrun_log.level = mfg_event_pb2.TestRunLogMessage.CRITICAL
    # pylint: enable=no-member
...
class UploadToMfgInspector(object):
  ...
  def upload_test_run(self, testrun):
    ...
    if isinstance(testrun, mfg_event_pb2.TestRun):
    ...
    test_run_envelope = quantum_data_pb2.UploadPackage()
