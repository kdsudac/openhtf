...
_RESOURCE_DIR = os.path.join(
    resources.GetARootDirWithAllResources(
        filename_predicate=lambda fn: 'openhtf/output/web_gui/' in fn,
        skip_previous_extraction_check=True),
    'google3', 'third_party', 'py', 'openhtf', 'output', 'web_gui')
BUILD_PATH = os.path.join(_RESOURCE_DIR, 'src', 'dist')
PREBUILT_PATH = os.path.join(_RESOURCE_DIR, 'prebuilt')
...
def main():
  ...
  if args.custom_frontend is not None:
    frontend_path = os.path.join(
        resources.GetARootDirWithAllResources(
            filename_predicate=lambda fn: args.custom_frontend in fn,
            skip_previous_extraction_check=True),
        args.custom_frontend
    )
  else:
